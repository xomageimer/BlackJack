...
bool TestBlackJack::PlayerConnect(const std::string &nick_name, std::shared_ptr<Actors::IPlayer> player) {
    return _grounds->SubscribePlayer(nick_name, std::move(player));
}

bool TestBlackJack::PlayerDisconnect(const std::string &nick_name) {
    return _grounds->UnSubscribePlayer(nick_name);
}

void TestBlackJack::Run(std::ostream &output) {
    _grounds->SetViewManager(std::make_shared<ConsoleLogger>(output));
    _grounds->Execute();
}

void TestBlackJack::Run(std::ostream &output, size_t command_count) {
    _grounds->SetViewManager(std::make_shared<ConsoleLogger>(output));
    _grounds->Execute(command_count);
}

TestBlackJack::TestBlackJack() {
    _grounds = std::make_shared<GameGround>(std::make_shared<OutputManager>());
}
